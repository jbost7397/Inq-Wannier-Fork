...
auto overlap_slate(const field_set_type & phi){
	...
	slate::HermitianMatrix<typename field_set_type::element_type> overlap_matrix(slate::Uplo::Lower, phi.set_size(), phi.set_part().block_size(), phi.full_comm().shape()[1], phi.full_comm().shape()[0], phi.full_comm().get());
