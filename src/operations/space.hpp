...
void to_fourier_heffte(basis::real_space const & real_basis, basis::fourier_space const & fourier_basis, InArray4D const & array_rs, OutArray4D && array_fs) {
	...
	for(int ist = 0; ist < size(array_rs[0][0][0]); ist++){
		...
		
		{
			...
			fft.forward((const std::complex<double> *) raw_pointer_cast(input.data_elements()), (std::complex<double> *) static_cast<complex *>(output.data_elements()));
		}
...
void to_real_heffte(basis::fourier_space const & fourier_basis, basis::real_space const & real_basis, InArray4D const & array_fs, OutArray4D && array_rs, bool normalize) {
	...
	for(int ist = 0; ist < size(array_rs[0][0][0]); ist++){
		...
		
		{
			...
			fft.backward((const std::complex<double> *) raw_pointer_cast(input.data_elements()), (std::complex<double> *) static_cast<complex *>(output.data_elements()), scaling);
		}
