...
#ifndef INQ__REAL_TIME__IMETRS
#define INQ__REAL_TIME__IMETRS
...
void im_etrs(double const time, double const dt, systems::ions & ions, systems::electrons & electrons, IonSubPropagator const & ion_propagator, ForcesType const & forces, CurrentType const & current,
	...
	for(auto & phi : electrons.kpin()){
		...
		auto halfstep_phi = operations::exponential_2_for_1(ham, complex(-dt/2.0, 0), complex(-dt, 0), phi);
	...
	for(int iscf = 0; iscf < nscf; iscf++){
		...
		for(auto & phi : electrons.kpin()) {
			...
			operations::exponential_in_place(ham, complex(-dt/2.0, 0), phi);
			iphi++;
...
#ifdef INQ_REAL_TIME_IMETRS_UNIT_TEST
#undef INQ_REAL_TIME_IMETRS_UNIT_TEST
