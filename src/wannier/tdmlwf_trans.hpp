...
private:
	...
	//mutable std::optional<basis::field_set<basis::real_space, complex, parallel::arbitrary_partition>> wavefunctions_;
...
void update(const states::orbital_set<basis::real_space, complex>& wavefunctions, CommType & comm) {
  	...
  	int n_states_global = wavefunctions_.set_size();
  	int n_states_local = wavefunctions_.local_set_size();
  	//int n_states_local = wavefunctions_.set_part().max_local_size();
	//std::cout << "N_states_local = " << n_states_local << std::endl;
  	...
  	if (wavefunctions_.set_part().parallel()) {
		...
		//for(auto ipart = 0; ipart < wavefunctions_.set_comm().size(); ipart++){
		for(; hypercubic_it != hypercubic_it.end(); ++hypercubic_it){
  			//auto global_mat = wavefunctions_.matrix();
			auto gmat = begin(*hypercubic_it);
  			auto cur_local = gmat[0].size();
			auto l_offset = wavefunctions_.set_part().start(cur_rank);
			//std::cout << "init_rank = " << init_rank << ", cur_rank = " << cur_rank << ", k_offset = " << k_offset << ", l_offset = " << l_offset << ", cur_local = " << cur_local << ", n_states_local = " << n_states_local << std::endl;
    			gpu::run(cur_local, n_states_local, [lmat=begin(loc_mat), gmat, ta=begin(trig_array), nbas, l_offset, k_offset, a=begin(a_)] GPU_LAMBDA (auto l_wf, auto k_wf) {
		...
		/*if (comm.rank() == 0){
			std::ofstream outfile("overlap_matrix.txt");
			for(int i = 0; i < 6; i++){
				for (int j = 0; j < n_states_global; j++){
					for (int k = 0; k < n_states_global; k++){
						outfile << real(a_[i][j][k]) << '\n';
					}
				}
			}
			outfile.close();
...
bool overlap(T1 epsilon, T2 i, T2 j, const systems::cell & cell_) const {
  	...
  	if (dist <= epsilon || dist >= (len - epsilon) ) return true;  //need sqrt(a0^2 + a1^2 + a2^2) for cell diagonal distance. Diagonal dist - epsilon for pbc
  	return false;
}
...
double spread2(T i, T j, const systems::cell & cell) {
  	assert(i >= 0 && i < wavefunctions_.set_size());
  	assert(j >= 0 && j < 3);
  	const complex c = adiag_[2*j][i];
  	const complex s = adiag_[2*j+1][i];
