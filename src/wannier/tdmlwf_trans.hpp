  ...
  if (wavefunctions_.set_part().parallel()) {
	...
	for (; hypercubic_it != hypercubic_it.end(); ++hypercubic_it){
		...
		auto l_offset = cur_rank * n_states_local;
    		...
    		gpu::run(n_states_local, n_states_local, [loc_mat = begin(mat), mat_it, tb = begin(trig_array), nbas, l_offset, k_offset, a = begin(a_), cell=begin(cell_dim)] GPU_LAMBDA (auto l_wf, auto k_wf) {
    ...
    gpu::run(n_states_global, n_states_global, [mat = begin(wavefunctions_.matrix()), tb = begin(trig_array), nbas, a = begin(a_), cell=begin(cell_dim)] GPU_LAMBDA (auto l_wf, auto k_wf) {
      for (int ibas = 0; ibas < nbas; ibas++){
...
bool overlap(T1 epsilon, T2 i, T2 j, const systems::cell & cell_) const {
  ...
  double len1 = cell_.diagonal_length();
