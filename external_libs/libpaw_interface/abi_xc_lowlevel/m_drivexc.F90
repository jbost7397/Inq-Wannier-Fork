...
!            This case is probably beyond machine precision considerations
   ...
   LIBPAW_ERROR('nspden>2 not allowed !')
...
!Determine quantities available in input arguments
 ...
 if (usegradient==1) then
   if (.not.present(grho2_updn)) then
     LIBPAW_ERROR('missing grho2_updn argument!')
   end if
   if (nvxcgrho>0) then
     if (.not.present(vxcgrho)) then
       LIBPAW_ERROR('missing vxcgrho argument!')
     end if
     has_gradient=.true.
   end if
 else if (nvxcgrho>0) then
   LIBPAW_ERROR('nvxcgrho>0 and usegradient=0!')
 end if
 if (uselaplacian==1) then
   if (.not.present(lrho_updn)) then
     LIBPAW_ERROR('missing lrho_updn argument!')
   end if
   if (nvxclrho>0) then
     if (.not.present(vxclrho)) then
       LIBPAW_ERROR('missing vxclrho argument!')
     end if
     has_laplacian=.true.
   end if
 else if (nvxclrho>0) then
   LIBPAW_ERROR('nvxclrho>0 and uselaplacian=0!')
 end if
 if (usekden==1) then
   if (.not.present(tau_updn)) then
     LIBPAW_ERROR('missing tau_updn argument!')
   end if
   if (nvxctau>0) then
     if (.not.present(vxctau)) then
       LIBPAW_ERROR('missing vxctau argument!')
     end if
     has_kden=.true.
   end if
 else if (nvxctau>0) then
   LIBPAW_ERROR('nvxctau>0 and usekden=0!')
 end if
 if (abs(order)>=2) then
...
!Check other optional arguments
 ...
 if(ixc==50) then
   ...
   if (size(fxcT)/=npts) then
     LIBPAW_ERROR('fxcT size must be npts!')
   end if
 end if
...
&    ixc==6.or.ixc==21.or.ixc==22.or.ixc==50) then
   LIBPAW_ALLOCATE(rhotot,(npts))
   LIBPAW_ALLOCATE(rspts,(npts))
   if(nspden==1)then
...
!>>>>> RPA correlation from Perdew-Wang
   ...
   if (order**2 <= 1) then
     LIBPAW_ALLOCATE(exci_rpa,(npts))
     LIBPAW_ALLOCATE(vxci_rpa,(npts,2))
     optpbe=3
...
!    PMA: second index of vxcrho is nspden while that of rpa is 2 they can mismatch
     ...
     LIBPAW_DEALLOCATE(exci_rpa)
     LIBPAW_DEALLOCATE(vxci_rpa)
   else if (order /=3) then
     LIBPAW_ALLOCATE(exci_rpa,(npts))
     LIBPAW_ALLOCATE(vxci_rpa,(npts,2))
     optpbe=3
...
!  minus hyb_mixing*exchange with PBE (optpbe=-2)
   ndvxc_x=8
   LIBPAW_ALLOCATE(exc_x,(npts))
   LIBPAW_ALLOCATE(vxcrho_x,(npts,nspden))
   LIBPAW_ALLOCATE(vxcgrho_x,(npts,nvxcgrho))
...
&     dvxcdgr=vxcgrho_x,exexch=my_exexch,grho2_updn=grho2_updn)
     ...
     LIBPAW_ALLOCATE(dvxc_x,(npts,ndvxc_x))
     optpbe=2 !PBE exchange correlation
     call xcpbe(exc,npts,nspden,optpbe,order,rho_updn,vxcrho,ndvxc,nd2vxc,&
     dvxcdgr=vxcgrho,dvxci=dvxc,grho2_updn=grho2_updn)
     optpbe=-2 !PBE exchange-only
...
&     d2vxci=d2vxc_x,dvxcdgr=vxcgrho_x,dvxci=dvxc_x,grho2_updn=grho2_updn)
     ...
     dvxc(:,1:ndvxc_x)=dvxc(:,1:ndvxc_x)-dvxc_x(:,1:ndvxc_x)*my_hyb_mixing
     LIBPAW_DEALLOCATE(dvxc_x)
     LIBPAW_DEALLOCATE(d2vxc_x)
   end if
   LIBPAW_DEALLOCATE(exc_x)
   LIBPAW_DEALLOCATE(vxcrho_x)
   LIBPAW_DEALLOCATE(vxcgrho_x)
...
!  Then renormalize B3LYP and subtract VWN3 contribution
   LIBPAW_ALLOCATE(exc_c,(npts))
   LIBPAW_ALLOCATE(vxcrho_c,(npts,nspden))
   if(order**2>1)then
     LIBPAW_ALLOCATE(dvxc_c,(npts,ndvxc))
   end if
   if(order**2>4)then
     LIBPAW_ALLOCATE(d2vxc_c,(npts,nd2vxc))
   end if
...
&     vxcrho_c,grho2=grho2_updn,vxcgr=vxcgrho,dvxc=dvxc_c,d2vxc=d2vxc,xc_functionals=xc_funcs_lyp)
   ...
   LIBPAW_DEALLOCATE(exc_c)
   LIBPAW_DEALLOCATE(vxcrho_c)
   if(allocated(dvxc_c))then
     LIBPAW_DEALLOCATE(dvxc_c)
   end if
   if(allocated(d2vxc_c))then
     LIBPAW_DEALLOCATE(d2vxc_c)
   end if
...
!Deallocate arrays
 ...
 if(allocated(rspts)) then
   LIBPAW_DEALLOCATE(rspts)
 end if
 if(allocated(zeta)) then
   LIBPAW_DEALLOCATE(zeta)
 end if
