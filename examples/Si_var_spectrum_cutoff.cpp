...
int main(int argc, char ** argv){
	...
	auto output = [&](auto data){
		...
		time[iter] = data.time();
		ham.uniform_vector_potential() = vector3<double, covariant>{0.001, 0, 0};
		auto J = observables::current(ions, el, ham);
		Jx_array[iter] = J[0];

		if(data.root()){
			std::ofstream file("Si" + std::to_string(x_repeats) + "_" + std::to_string(y_repeats) + "_" + std::to_string(z_repeats) +  "_cur_cutoff.dat", std::ios_base::app);
			file << iter << '\t' << J[0] << '\t' << J[1] << '\t' << J[2] << std::endl;
		}

 		if(data.root() and data.every(1000)){
 			auto spectrum = observables::spectrum(20.0_eV, 0.01_eV, time({0, iter - 1}), Jx_array({0, iter - 1}));

    			std::ofstream file("Si" + std::to_string(x_repeats) + "_" + std::to_string(y_repeats) + "_" + std::to_string(z_repeats) + "_spectrum_cutoff.dat");

    			for(int ifreq = 0; ifreq < spectrum.size(); ifreq++){
     				file << ifreq*0.01 << '\t' << spectrum[ifreq] << '\t' << real(spectrum[ifreq])  << '\t' << imag(spectrum[ifreq]) << std::endl;
    			}
  		}
	};

        real_time::propagate(ions, el, output, functional, options::real_time{}.num_steps(niter).dt(dt).tdmlwf(mlwf_freq).epsilon(15.0_b), kick);
