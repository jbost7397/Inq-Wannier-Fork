...
int main(int argc, char ** argv){
	...
	auto kick = perturbations::kick(ions.cell(), {0.001, 0.0, 0.0}, perturbations::gauge::velocity);
	...
	auto output = [&](auto data){
		...
		dip[iter] = data.dipole()[0];
                ...
                if (data.root()) {
                        std::ofstream tfile("time_benz_mlwf.dat", std::ios::app);
                        std::ofstream dfile("dipole_benz_mlwf.dat", std::ios::app);
                        std::ofstream efile("energy_benz_mlwf.dat", std::ios::app);
                        std::ofstream cfile("current_benz_mlwf.dat", std::ios::app);
	...
	real_time::propagate(ions, electrons, output, functional, options::real_time{}.num_steps(nsteps).dt(timestep).tdmlwf(), kick);
