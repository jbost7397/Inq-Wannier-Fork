...
int main(int argc, char ** argv){
	...
	auto output = [&](auto data){
		...
		if(data.root()){
			std::ofstream file("dipole_mlwf.dat", std::ios_base::app);
 		...
 		if(data.root() and data.every(1000)){
 			auto spectrum = observables::spectrum(20.0_eV, 0.01_eV, time({0, iter - 1}), dip({0, iter - 1}));
    			...
    			std::ofstream file("spectrum_mlwf.dat");
	...
	real_time::propagate(ions, electrons, output, functional, options::real_time{}.num_steps(nsteps).dt(timestep).tdmlwf(mlwf_freq), kick);
