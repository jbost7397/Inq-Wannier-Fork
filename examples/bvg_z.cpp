...
int main(int argc, char ** argv){
	...
	auto kick = perturbations::kick(ions.cell(), {0.0, 0.0, 0.001}, perturbations::gauge::velocity);
	...
	auto output = [&](auto data){
		...
		time[iter] = data.time();
		cur[iter] = data.current()[2];
		en[iter] = data.energy().total();
		dip[iter] = data.dipole()[2];

                if (data.root()) {
                        std::ofstream tfile("time_benz_z.dat", std::ios::app);
                        std::ofstream dfile("dipole_benz_z.dat", std::ios::app);
                        std::ofstream efile("energy_benz_z.dat", std::ios::app);
                        std::ofstream cfile("current_benz_z.dat", std::ios::app);
