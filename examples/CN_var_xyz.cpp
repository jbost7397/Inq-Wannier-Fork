...
int main(int argc, char ** argv){
	...
	systems::electrons el(env.par().states().domains(pardomains), ions, options::electrons{}.cutoff(ecut).extra_states(extra_states), input::kpoints::grid({nk, nk, nk}));
        ...
        if(not_found_gs){
                ...
                try { inq::ground_state::calculate(ions, el, functional, inq::options::ground_state{}.energy_tolerance(1e-6_Ha).mixing(mix).max_steps(max).broyden_mixing()); }
                catch(...){ }
                el.save(restart_dir);
        }

        real_time::propagate(ions, el, [](auto){}, functional, options::real_time{}.num_steps(niter).dt(dt).tdmlwf(mlwf_freq).crank_nicolson());
